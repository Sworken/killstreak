...
void OnPVPKill(Player *pKiller, Player *pVictim)
{
    ...
    else if(PvPSystemEnabled)
    {
      ...
      if(LooseTokenOnPvPDeath == true)
        ...
        case KillerStreak2:
        sprintf(msg, "[PvP System]: %s [DoubleKill] killed %s and is on a %u kill streak. ", pKiller->GetName(), pVictim->GetName(), KillStreaks[2]);
        sWorld->SendWorldText(LANG_SYSTEMMESSAGE, msg);
        ...
        case KillerStreak4:
        sprintf(msg, "[PvP System]: %s [MonsterKill] killed %s and is on a %u kill streak. ", pKiller->GetName(), pVictim->GetName(), KillStreaks[4]);
        sWorld->SendWorldText(LANG_SYSTEMMESSAGE, msg);
        pKiller->AddItem(ItemReward, AmountOfRewardsOnKillStreak[4]);
        break;

        case KillerStreak5:
        sprintf(msg, "[PvP System]: %s [MonsterKill] killed %s and is on a %u kill streak. ", pKiller->GetName(), pVictim->GetName(), KillStreaks[5]);
        sWorld->SendWorldText(LANG_SYSTEMMESSAGE, msg);
